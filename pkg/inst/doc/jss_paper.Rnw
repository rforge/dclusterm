...
@
\caption{**INCLUDE TCE LOCATIONS** SMR of the incidence of Leukemia in upstate New York.}
\label{fig:NYmap}
\end{figure}
...
<<results=hide>>=
m0 <- glm(Observed ~ offset(log(Expected)) + 1, family = "poisson", data = NY8)
...
<<>>=
m1<-glm(Observed ~ offset(log(Expected))+PCTOWNHOME+PCTAGE65P+PEXPOSURE, 
...
\noindent
\noindent
where $\mu_{i,t}$ is the mean of area $i$ at time $t$ and $c^{(j)}_{i,t}$
...
brainnav$CLUSTER<-as.factor(knbinary(brainnav, cl0)[,1])
