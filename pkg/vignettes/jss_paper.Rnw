...
to extend these ideas to detect clusters in space and time. The detection of
...
outside the cluster. By including cluster covariates in the mode we obtain an
estimate of the increased risk (as measured by its associated coefficient) and
its significant (by means of the associated p-value, for example).
...
that, we are only interested in clusters whose associated coefficient is
...
\code{get.allkncluster()} to get the indices of the aras in the clusters and
...
the time period have been constrained to 1985--1989.  Furthermore,
...
@
\caption{Spatio-temporal most significantly cluster of brain cancer detected in New Mexico.}
\label{fig:NMcluster}
\end{figure}
...
will be 1 if the corresponding  areas are in a cluster and 0 otherwise. This indices can
...
cluster detected before has now dissappeared and only the two smallest clusters
...
one to be declared as non-singnificant.  The two remaining clusters have the
...
As in the example in Section~\ref{subsec:ex:NY}, we have created two clusters variables
