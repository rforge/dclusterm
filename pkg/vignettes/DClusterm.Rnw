...
detection of spatial clusters with case-control point data are avilable in
...
Hence, our first action is to load some required packages, such as \pkg{xts} \citep{xts:2014}, and
the dataset itself.
...
to the nearest TCE site, \code{PCTAGE65P} is the percentage people aged 65 or more,
and \code{PCTOWNHOME} is the percentage of people who own their home. The red
...
\textbf{Perhaps remove typeCluster from the list of arguments and
set it inside DetectClustersModel depending on whether a sp or
spacetime object is passed.}


The output of \code{DetectClustersModel()} returns all clusters found at
...
variance should be equal. Hence, by including random effects in a Poisson model
...
to the inclusion of  random effects in the model.
...
\Sexpr{round(summary(nav.m0q)$dispersion)}, which is
...
and these will be the only considered in the analysis.
...
Now, the same \Sexpr{nrow(nm.cl1.s)}  clusters are detected.  Note how these
clusters only cover a few areas, but over several years.
