...
\And Paula Moraga-Serrano\\Queensland University of Technology
...
dummy variables and then fitting many generalized linear models to the data where
...
certain percentage of the population has been reached (defined by argument
...
\Sexpr{nrow(ny.clmm0)}. These are shown in Figure~\ref{}. The largest cluster
detected before has now dissappeared and only the two smallest clusters remain.
This may be due to the fact that XXXXXXXX.  The two remaining clusters have the
...
#No covariates
plot(NY8)
points(ny.clmm0[,1:2], pch=19, col="red")

#With covariates
plot(NY8)
points(ny.clmm1[,1:2], pch=19, col="red")
@
\caption{Clusters detected with no covariate adjustment (left) and after adjusting for
  covariates (right) using a mixed-effects model to account for overdispersion
of the data.}
\label{fig:NYclmm}
\end{figure}
