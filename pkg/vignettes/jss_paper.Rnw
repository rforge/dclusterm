...
available. Generalized Linear Models \citep[GLM,][]{McCullaghNelder:1989} are a
...
$$
\noindent
Here, $\gamma_j$ is a measure of the risk in the cluster. We are only
...
ignored.  Testing different clusters will produce many different cluster
covariates.  We can use model selection techniques to select the most important
cluster in the study region. In particular, the log-likelihood can be used to
compare the model with the cluster variable to the null model (i.e., the one
...
include in this paper we will always consider both scenarios to better
...
have been analyzed using a similar methodology.  In particular, we have
