...
multiple testing as only one possible cluster a given area is reported,
similarly as the SSS.
...
that it is also posible to look for spatial clusters by aggregating observed
