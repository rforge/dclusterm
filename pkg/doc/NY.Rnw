...
cl0<-DetectClustersModel(NY8st, thegrid=as.data.frame(NY8)[1:2,c("x", "y")], 
   fractpop=.15, alpha=1, radius=Inf, step=NULL,
...
<<results=hide>>=
cl1<-DetectClustersModel(NY8st, thegrid=as.data.frame(NY8)[1:2,c("x", "y")], 
   fractpop=.15, alpha=1,
