...
*** analyse the incidence of brain cancer in Navarre (Spain) in XXXX.  The
aggregation level is the health district. Figure \ref{fig:Navarre}  shows the
SMR. As it can be seen there are many areas where the SMR is zero because there
are no cases in those areas. *** also tested for positive zero-inflation of
these data compared to a Poisson distribution.
...
@
\label{fig:Navarre}
\caption{SMR of brain cancer in Navarre (Spain) in XXXX.}
\end{figure}
...
Futheremore, a quasipoisson model has been fit in order to asses any
...
For this reason, and following *** a zero-inflated Poisson model has been
fit. Here is the resulting model:
